...
- Movement: Accept W, A, S, D key input from user.  Once done the game can update player position based on the key pressed or return error message for invalid input.
...
- Timer: The game will have a time limit. Once done the game will keep count of time and will end if the timer goes to 0.
- Display: Display maze, timer, and other useful information. Once done, the user will be prompted for input and be able to see the maze and timer.
...
# Collaboration 
Pair programming: Initialize maze class
Individual: main file, maze class, and timer class
